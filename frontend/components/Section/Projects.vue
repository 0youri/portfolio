  ...
  <section class="text-white py-10  text-center">
    ...
    >
      <div
        ...
        :class="item.background || 'bg-slate-800'"
        :style="item.pictures ? { backgroundImage: `url('${getImgUrl(item.pictures[0])}')` } : {}"
      >
        <div v-if="item.pictures" class="absolute inset-0 bg-black/80 z-0 rounded-3xl"></div>
          ...
          <div class="flex flex-col items-center">
            ...
            >
              <UBadge
                ...
                color="neutral" :variant="item.pictures ? 'soft' : 'subtle'" size="sm"
              ...
              <template #body>
                <div class="h-full space-y-5">
                  <UCarousel
                    ...
                    :items="item.pictures"
                    :ui="{
                      item: 'basis-full transition-opacity [&:not(.is-snapped)]:opacity-10'
                    }"
    ...
    <UButton
      ...
      class="mx-auto mt-5 text-sm font-medium transition-transform duration-300 hover:scale-[1.03]"
      to="/projects"
    >
